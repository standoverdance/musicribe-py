...
class Menu():
        ...
        for button in self.buttons:  # Only check for buttons in the active list
            if button.is_clicked(event):
                ...
                if button == self.b_play:
                    ...
                    #self.selected_level_data = "../data/2212704 toby fox - Snowy/toby fox - Snowy (_HeLLFly_) [4K Easy].osu"
                ...
                for file_name in os.listdir(folder_path):
                    if file_name.endswith('.osu'):
                        ...
                        with open(osu_path, 'r', encoding='utf-8') as file:
                            _, _, _, text_exist, audio, image = self.interate_on_osu(file)
                            if text_exist:
                                audio_path = folder_path +"/" + str(audio)
                                image_path = folder_path +"/"+ image
                                self.levels.append(Preview(osu_path, audio_path, image_path))
    ...
    def interate_on_osu(self, file):
        ...
        return object_count, mode, updated_lines, text_exists, audio, bgimage
    
    def file_open_and_rmv(self, path): # Removes the possible duplicates
        with open(path, 'r', encoding='utf-8') as file:
            self.object_count, mode, updated_lines, _, _, _ = self.interate_on_osu(file)
        ...
        if mode == 3:
            with open(path, 'w', encoding='utf-8') as file:
                ...
                file.write('\n'.join(updated_lines))
            with open(path, 'r', encoding='utf-8') as file:
                self.object_count, _, _, _, _, _ = self.interate_on_osu(file)  # Get the updated count
